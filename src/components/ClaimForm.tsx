...
function ClaimForm(props: any) {
  ...
  const [toRelease, setToRelease] = useState<boolean>(false);
  ...
  const subscribeToRouterContractEvents = async () => {
    ...
    await RouterContract.on(
      ...
      (claimer, amount, wrappedTokenAddr, event) => {
        ...
        console.log("TokensClaimed caughtt!");
  ...
  const subscribeToERCContractEvents = async (tokenAddress: string) => {
    if (isAddress(tokenAddress)) {
      ...
      // tslint:disable: no-console
      console.log("Listening to transferback events");

      await ERCContract.on("Approval", (owner, spender, value, event) => {
        if (spender === signerAddress) {
          // tslint:disable: no-console
      console.log("Caught transferback events");
          setPendingState(false);
  ...
  const checkTokenClaimed = async (nativeToken:string) => {
    ...
    const routerAddr =
      network.name === ROPSTEN_NETWORK
        ...
        const wrappedTokenAddr = await RouterContract.getWrappedTokenAddress(nativeToken);
        // tslint:disable: no-console
        console.log("Token ADDR ", wrappedTokenAddr.toString());
        const wrappedTokenExists = wrappedTokenAddr.toString() !== "0x0000000000000000000000000000000000000000";
       
        if(wrappedTokenExists) {
          const signer = provider.getSigner();
          const signerAddress = await signer.getAddress();
          const wrappedTokensAmount = await RouterContract.getTokensClaimedAmount(wrappedTokenAddr.toString(), signerAddress);
  ...
  const onChangeTokenAddress = async (e: any) => {
    ...
    if (isAddress(addr)) {
      setIsValidAddress(true);
      const code = await provider.getCode(addr); // should be '0x' because we enter native token and we are on the opposite network
      if(props.hasUnwrapped) {
        await subscribeToERCContractEvents(addr);
      } else if (props.hasLocked && claimedAmount === -1) { // locked but hasn't claimed
  ...
  const enableMint = function() {
    if(lockedAmount !== 0 && claimedAmount === -1) { // if locked but hasn't claimed any
  ...
  return (
    <div id="claimForm">
      ...
      <FormGroup>
        {/* We want the claim button disabled iff:  1. Address is invalid or 2. Address is null or 3. If tokens are locked, disable the button if amount != lockedAmount, otherwise enable it*/}
      ...
      <div id="transactionInfo">
        ...
        {(lockedAmount !== amount && props.hasLocked && !props.hasUnwrapped && (claimedAmount !== lockedAmount)) && (
          <p id="tokenLocked">
            {" "}
