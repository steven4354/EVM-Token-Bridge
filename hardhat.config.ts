...
task("deploySatoshi", "Deploys the contract", async (taskArgs, hre) => {
  const Satoshi = await hre.ethers.getContractFactory("SatoshiToken"); 
  const SatoshiContract = await Satoshi.deploy();
  // tslint:disable: no-console
  console.log('Waiting for SatoshiToken deployment...');
  await SatoshiContract.deployed();
  // tslint:disable: no-console
  console.log("ERC20Permit deployed to:", SatoshiContract.address);
});
